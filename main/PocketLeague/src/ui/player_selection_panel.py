    ...
    def draw(self, surface):
        if self.__active:
            ...
            for label in self.__choice_labels:
                label.draw_to(surface)
            pygame.draw.circle(surface, TEAM_COLOR_MAP[self.__possible_values[1][self.__indexes[1]]], self.__offset(PLAYER_SELECTION_PANEL_PREVIEW_OFFSET), PLAYER_OUTER_RADIUS)
            pygame.draw.circle(surface, COLOR_MAP[self.__possible_values[2][self.__indexes[2]]], self.__offset(PLAYER_SELECTION_PANEL_PREVIEW_OFFSET), PLAYER_INNER_RADIUS)
        else:
